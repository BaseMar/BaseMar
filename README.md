# Hi, I'm Martino 👋

### Entry-Level Data Analyst | Python | SQL | Power BI | Streamlit | Data Automation

I'm developing my skills in data analysis, Python automation, databases, and analytical dashboards.  
I like building practical projects that turn raw data into useful insights, clear visualizations, and decision-support tools.

Currently, I focus on:

- 🐍 Python for data analysis, automation, and data processing
- 📊 Power BI dashboards, DAX measures, and business reporting
- 🗄️ SQL, PostgreSQL, and relational data modeling
- 🤖 Machine learning basics, recommendation systems, and AI-assisted analytics
- 🚀 Building portfolio projects that solve real analytical problems

---

## 👨‍💻 About Me

- 🎯 I am focused on becoming a **Data Analyst / Python Data Automation Specialist**
- 🧠 I work with **Python, SQL, Pandas, Streamlit, Plotly, PostgreSQL, Power BI, and DAX**
- 📈 I enjoy projects related to **fitness analytics, personal finance, dashboards, and data-driven recommendations**
- 🛠️ I build end-to-end projects: from data generation/collection, through processing and modeling, to visualization
- 🎓 I have completed postgraduate studies in **Data Analysis / Data Science with AI Elements**
- 🔍 I am looking for opportunities where I can grow in **data analysis, automation, reporting, and analytics engineering**
- 🏋️ In my free time, I train at the gym and often combine this interest with data projects

---

## 🚀 Featured Projects

### 🏋️ Workout Progress Analyzer

End-to-end workout analytics dashboard built with **Python, Streamlit, PostgreSQL, Pandas, Plotly, and automated tests**.

The application helps analyze:

- strength progress and regressions
- training consistency
- fatigue patterns
- muscle-group volume distribution
- body composition and measurement trends

🔗 Repository: [ProgressAnalyzer](https://github.com/BaseMar/ProgressAnalyzer)  
🌐 Live demo: [Streamlit App](https://progressanalyzer-jysznkmwcjnejwamgmlfqd.streamlit.app)

---

### 🤖 Training AI Project

Portfolio Data Science project focused on synthetic workout data, exploratory data analysis, machine learning regression models, and a hybrid recommendation engine.

The project includes:

- synthetic training data generator
- EDA and feature engineering
- model comparison and prediction pipeline
- recommendation engine for training parameters
- Streamlit dashboard for presenting results

🔗 Repository: [training-ai-project](https://github.com/BaseMar/training-ai-project)

---

### 💰 Personal Finance Intelligence Dashboard

Power BI dashboard for analyzing personal income, expenses, budget usage, savings rate, and spending patterns.

The project focuses on:

- clean data model design
- Power Query transformations
- DAX measures
- budget and savings analysis
- financial recommendations based on spending behavior

🔗 Repository: [personal-finance-dashboard-powerbi](https://github.com/BaseMar/personal-finance-dashboard-powerbi)

---

## 🛠️ Skills & Tools

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=for-the-badge&logo=plotly&logoColor=white)

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![MS SQL](https://img.shields.io/badge/MS_SQL-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![DAX](https://img.shields.io/badge/DAX-276DC3?style=for-the-badge&logo=powerbi&logoColor=white)

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![PyCharm](https://img.shields.io/badge/PyCharm-000000?style=for-the-badge&logo=pycharm&logoColor=white)

---

## 📚 Currently Learning

- Advanced SQL and data modeling
- Power BI dashboard design and DAX
- Machine learning workflows with scikit-learn
- Data automation and API integrations
- Building better portfolio projects for Data Analyst roles

---

## 📫 Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Martino%20Sebastiani-blue?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/martino-sebastiani-b17028279/)
[![Email](https://img.shields.io/badge/Email-msmsebastiani%40gmail.com-red?style=for-the-badge&logo=gmail&logoColor=white)](mailto:msmsebastiani@gmail.com)
